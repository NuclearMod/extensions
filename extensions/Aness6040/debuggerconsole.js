...
((Scratch) => {
    ...
    function addText({ message, color, bullet = '', innerHTML = false }) {
        ...
        // Appeler la fonction autoScroll pour faire défiler automatiquement vers le bas
        autoScroll(logElement.parentNode);
    ...
    addText({
        color: '#808080',
        message: `<h1>Debugger Console</h1><br><h3>v${version}</h3><br>Originally by <a href="https://github.com/NOname-awa/">NOname-awa</a> (Based on turboWarp CN) and continued by <a href="https://github.com/Aness6040/">Aness6040</a> (actual extension).<br><br>` + lang('aness6040debugger.tipForHelp', 'Type "help" to view > help.'),
    });
    ...
    class Aness6040debugger {
        getInfo() {
            ...
            blockIconURI: extIcon,
                ...
                blocks: [
                    ...
                    {
                        opcode: 'outputWithANSIEncode',
                        blockType: BlockType.COMMAND,
                        text: lang('aness6040debugger.outputWithANSIEncode', 'output [message] with ANSI encode'),
                        arguments: {
                            message: {
                                type: ArgumentType.STRING,
                                defaultValue: ''
        ...
        ansiToHtml(ansiText) {
            ...
            const ansiToHtmlMap = {
                '0': '</span>',  // Reset
                '1': 'font-weight: bold;',
                '2': 'color: darkgreen;',  // Faint (not widely supported)
                '3': 'font-style: italic;',
                '4': 'text-decoration: underline;',
                '5': 'text-decoration: blink;',  // Blink (not widely supported)
                '7': 'background-color: white; color: black;',  // Inverse
                '8': 'color: transparent;',  // Hide (not widely supported)
                '9': 'text-decoration: line-through;',  // Strikethrough (not widely supported)
                '10': 'font-family: initial;',  // Default font
                '21': 'text-decoration: double underline;',  // Double underline
                '22': 'font-weight: normal;',  // Bold off
                '23': 'font-style: normal;',  // Italic off
                '24': 'text-decoration: none;',  // Underline off
                '28': 'visibility: visible;',  // Hidden off
                ...
                '36': 'color: cyan;',
                '37': 'color: white;',
                '38': '',  // 8-bit foreground color
                '39': 'color: initial;',  // Default foreground color
                ...
                '48': '',  // 8-bit background color
                '49': 'background-color: initial;',  // Default background color
                '51': 'border: 1px solid;',  // Frame (not widely supported)
                '53': 'text-decoration: overline;',  // Overline (not widely supported)
                '54': 'border: none;',  // Frame or encircle off
                '55': 'text-decoration: none;',  // Overline off
                '58': 'text-decoration: underline; text-decoration-color:',  // Set underline color (requires additional handling)
                '59': 'text-decoration-color: initial;',  // Default underline color
                '73': 'vertical-align: super;',  // Superscript
                '74': 'vertical-align: sub;',  // Subscript
                '75': 'font-weight: bold; font-size: larger;',  // Added new property (example effect)
                '90': 'color: rgb(128, 128, 128);',  // Bright black (not widely supported)
                ...
                '100': 'background-color: rgb(128, 128, 128);',  // Bright black background (not widely supported)
                '101': 'background-color: rgb(255, 0, 0);',  // Bright red background
                '102': 'background-color: rgb(0, 255, 0);',  // Bright green background
                '103': 'background-color: rgb(255, 255, 0);',  // Bright yellow background
                '104': 'background-color: rgb(0, 0, 255);',  // Bright blue background
                '105': 'background-color: rgb(255, 0, 255);',  // Bright magenta background
                '106': 'background-color: rgb(0, 255, 255);',  // Bright cyan background
                '107': 'background-color: rgb(255, 255, 255);'  // Bright white background
            };
        
            function get8BitColorCode(code) {
                if (code >= 0 && code <= 7) {
                    // Standard colors
                    return ['black', 'red', 'green', 'yellow', 'blue', 'magenta', 'cyan', 'white'][code];
                } else if (code >= 8 && code <= 15) {
                    // High-intensity colors
                    return ['gray', 'lightred', 'lightgreen', 'lightyellow', 'lightblue', 'lightmagenta', 'lightcyan', 'white'][code - 8];
            ...
            // Replace newlines encoded as '\n' and '\\n' with <br> before processing ANSI codes
            ansiText = ansiText.replace(/\\n/g, '<br>').replace(/\n/g, '<br>');

            ansiText = ansiText.replace(/\\x1b/g, '\x1B')
            ansiText = ansiText.replace(/\\x1B/g, '\x1B')
            ansiText = ansiText.replace(/\\003/g, '\x1B')
            ansiText = ansiText.replace(/\\e/g, '\x1B');
        
            let htmlText = ansiText.replace(/\x1B\[((?:\d+;?)+)m/g, (match, p1) => {
                const codes = p1.split(';');
                let styles = codes.map((code, index) => {
                    if (code >= 30 && code <= 37) {
                        return ansiToHtmlMap[code] || '';
                    } else if (code >= 40 && code <= 47) {
                        return ansiToHtmlMap[code] || '';
                    } else if ((code === '38' || code === '48') && codes.length > index + 2 && codes[index + 1] === '5') {
                        const colorCode = get8BitColorCode(parseInt(codes[index + 2]));
                        if (colorCode) {
                            if (code === '38') {
                ...
                // Handle special case for underline color
                if (styles.includes('text-decoration: underline; text-decoration-color:')) {
                    const colorCode = codes[1] || 'initial';
                    styles = `text-decoration: underline; text-decoration-color: ${colorCode};`;
                }
            
                if (styles.includes('</span>')) {
                    return '</span>';
                } else {
            ...
            // Replace all remaining '\033[0m' with '</span>' to ensure closure
            htmlText = htmlText.replace(/\x1B\[0m/g, '</span>')
        
            // Ensure all open spans are closed at the end
            const openSpanCount = (htmlText.match(/<span/g) || []).length;
            const closeSpanCount = (htmlText.match(/<\/span>/g) || []).length;
            htmlText += '</span>'.repeat(Math.max(0, openSpanCount - closeSpanCount)) + '<span>'.repeat(Math.max(0, closeSpanCount - openSpanCount));
        ...
        outputWithANSIEncode({message}){
            console.log(message);
            console.log(this.ansiToHtml(message))
            addText({message: this.ansiToHtml(message)});
        }
